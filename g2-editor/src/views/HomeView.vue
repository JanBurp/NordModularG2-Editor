<template>
	<div class="max-w-xl mx-auto">
		...
		<div v-else>
			...
			<Button variant="secondary" @click="handleFetchSettings"
			...
			<div v-if="device.settings" class="mt-6">
				...
				<Card title="Slots">
					<div
						v-for="slot in device.settings.slots"
						:key="slot.slot"
				...
				<Card v-if="device.settings.patches" title="Patch">
					...
					<p>
						Clock:
						{{ device.settings.patches.clockRunning ? "Running" : "Stopped" }}
					</p>
				</Card>
				...
				<Card v-if="device.settings.performance" title="Performance">
					...
					<p>
						Clock:
						{{
							device.settings.performance.clockRunning ? "Running" : "Stopped"
						}}
					</p>
				</Card>
			</div>
