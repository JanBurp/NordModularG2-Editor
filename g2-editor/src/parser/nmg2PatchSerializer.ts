...
class BitWriter {
	...
	write(numbits: number, value: number): void {
		for (let bw = Math.pow(2, numbits - 1); bw >= 1; bw >>= 1)
			this.bits.push(value & bw ? 1 : 0);
	}
	...
	for (const m of withParams) {
		...
		for (let v = 0; v < 9; v++) {
			...
			for (let p = 0; p < m.pcnt; p++)
				bw.write(7, m.lv[v * m.pcnt + p] ?? 0);
		}
	}
	return makeSection(0x4d, bw.flush());
}

function writeModuleNames(
	areaIdx: 0 | 1,
	modules: ModuleInstance[],
): Uint8Array | null {
	...
	for (const m of named) {
		...
		for (let i = 0; i < nameLen; i++)
			data[ofs + 1 + i] = m.uname!.charCodeAt(i);
...
export function serializePatch(
	...
	while (ofs < sectionDataLen) {
		...
		if (type === 0x4a && areaIdx <= 1 && !written4a.has(areaIdx)) {
			outSections.push(
				writeModuleList(areaIdx as 0 | 1, patch.areas[areaIdx].modules),
			);
			...
			outSections.push(
				writeCableList(
					areaIdx as 0 | 1,
					patch.areas[areaIdx].cableList ?? [],
				),
			);
			written52.add(areaIdx);
		} else if (type === 0x4d && areaIdx <= 1 && !written4d.has(areaIdx)) {
			outSections.push(
				writeParameters(areaIdx as 0 | 1, patch.areas[areaIdx].modules),
			);
			written4d.add(areaIdx);
		} else if (type === 0x5a && areaIdx <= 1 && !written5a.has(areaIdx)) {
			const namesSection = writeModuleNames(
				areaIdx as 0 | 1,
				patch.areas[areaIdx].modules,
			);
