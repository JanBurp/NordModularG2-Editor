...
function pch2_(data: ArrayBuffer, filename: string) {
	...
	function findModule(areaIdx: number, index: number): ModuleInstance | null {
		for (let i = 0; i < areas[areaIdx].modules.length; i++) {
			if (areas[areaIdx].modules[i].index == index)
				return areas[areaIdx].modules[i];
		}
		return null;
	}
	...
	function parseModuleList(data: Uint8Array) {
		...
		for (let i = 0; i < nummod; i++) {
			...
			if (!modDef) {
				console.error(
					`Module type ${modtype} not found in module definitions`,
				);
				continue;
			}
	...
	this.writeParameters = (areanum: number) => {
		...
		const filedataArray = new Uint8Array(
			self.data,
			ofs + 1,
			dataArray.byteLength - ofs - 3,
		);
		const calcCRC = calcCrc(filedataArray);
		dataArray.set(
			[Math.floor(calcCRC / 256), calcCRC % 256],
			self.data.byteLength - 2,
		);
	};
	...
	function getModuleByName(
		...
		for (let i = 0; i < areas[areaIdx].modules.length; i++)
			if (areas[areaIdx].modules[i].uname == name)
				return areas[areaIdx].modules[i];
		return null;
	}
	...
	const g2section: Record<
		number,
		[string, (data: Uint8Array) => string | undefined]
	> = {
	...
	while (ofs < maxofs) {
		...
		if (type in g2section) {
			res +=
				...
				g2section[type][0] +
				':offset=0x' +
				(textHdrLen + ofs).toString(16);
			if (g2section[type][1])
				res += g2section[type][1](
					new Int8Array(data, textHdrLen + ofs + 3, siz),
				);
		}
		ofs += siz + 3;
		if (type == 0x6f) {
...
export class PatchParser {
	...
	constructor(buffer: ArrayBuffer) {
		this.patcher = new pch2_(buffer, '') as unknown as ReturnType<
			typeof pch2_
		>;
	}
	...
	parse(): Patch {
		return {
			areas: [
				this.patcher.getArea(0) as Area,
				this.patcher.getArea(1) as Area,
			],
