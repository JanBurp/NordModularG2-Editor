/*  Clavia Nord Modular G2 pch2 file reader writer
    ...
    sections for all slots follow with each terminating with a 0x6F text pad.
    Because we don't know that we have a prf2 (in case of php attachment loader as source), we should pass the slot names
    into the pch_ object and provide an 'isPrf()' function that will return an array of slot names if true, or null.
    After processing the first slot data we can simply add another "fx" and "va" area for each new slot data
*/
...
function pch2_(data, filename) {
    ...
    this.getAllModules = (areaIdx) => areas[areaIdx].modules;

    this.getAllCables = (areaIdx) => areas[areaIdx].cableList;

    this.getTextPad = () => {
        if (textpadlen == 0) return null;
        let rv = "";
        const dv = new DataView(self.data);
        for (let i = 0; i < textpadlen; i++)
    ...
    this.setTextPad = (str) => {
        ...
        const dv = new DataView(self.data);
        for (let i = 0; i < Math.min(str.length, textpadlen); i++)
            dv.setUint8(textpadofs + i, str.charCodeAt(i));
    };
    this.getArea = (areaIndex) => areas[areaIndex];
    ...
    function getBits(numbits, initialData, maxreq) {
        if (initialData) {
            ...
            for (let i = 0; i < max; i++)
                for (let j = 0x80; j > 0; j = j >> 1)
                    bitArray.push(initialData[i] & j ? 1 : 0);
            bitofs = 0;
        }
    ...
    function setBits(numbits, byte) {
        ...
        if (numbits) {
            for (let bw = Math.pow(2, numbits - 1); bw; bw = bw >> 1)
    ...
    function findModule(areaIdx, index) {
    ...
    function parseModuleList(data) {
        ...
        for (let i = 0; i < nummod; i++) {
            ...
            const mod = Object.create(window.modules.getById(modtype));
            mod.pcnt = mod.params.length;
    ...
    const pd = [];
    function parsePatchDesc(data) {
        const description_attrs = {
            voices: 5, height: 14, unk2: 3,
            red: 1, blue: 1, yellow: 1, orange: 1,
            green: 1, purple: 1, white: 1,
            monopoly: 2, variation: 8, category: 8,
        };
        getBits(7 * 8 + 5, data);
        for (const a in description_attrs)
            description_attrs[a] = getBits(description_attrs[a]);
        pd[aof >> 1] = description_attrs;
    }
    this.getpd = (slot) => pd[slot];

    this.writeParameters = (areanum) => {
        ...
        dataArray.set(setBits(0), area.paramaterDataOfs);
        const ofs = this.ofs;
    ...
    function getModuleByName(areaIdx, name) {
        for (let i = 0; i < areas[areaIdx].modules.length; i++)
            if (areas[areaIdx].modules[i].uname == name) return areas[areaIdx].modules[i];
        return null;
    }
    this.getModuleArray = (areaIdx, basename) => {
        const array = [];
        let i = 0;
        let m;
    ...
    const str = String.fromCharCode.apply(null, hdr);
...
window.PatchParser = PatchParser;
export { PatchParser };
