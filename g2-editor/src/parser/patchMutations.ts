...
export function mutDeleteModule(
	patch: Patch,
	areaIdx: 0 | 1,
	moduleId: number,
): void {
...
export function mutSetModuleColor(
	patch: Patch,
	areaIdx: 0 | 1,
	moduleId: number,
	color: number,
...
export function mutSetModuleLabel(
	patch: Patch,
	areaIdx: 0 | 1,
	moduleId: number,
	label: string,
	...
	patch.areas[areaIdx].modules = patch.areas[areaIdx].modules.map((m) =>
		m.index === moduleId ? { ...m, uname: label } : m,
	);
}
