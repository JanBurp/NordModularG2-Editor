...
export function makePatchCables(
  ...
  cablesToRender.forEach((cable) => {
    ...
    const smodDef = (window as any).modules?.getById(smod.type) as ModuleDef | undefined;
    const dmodDef = (window as any).modules?.getById(dmod.type) as ModuleDef | undefined;
...
export function removeAllCables(svgElement: SVGElement): void {
  ...
  while (sibling) {
    ...
    if (sibling.tagName === 'path' && 
        (sibling.classList.contains('svgcable') || sibling.classList.contains('svgcableborder'))) {
