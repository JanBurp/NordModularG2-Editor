...
<template>
	<div class="flex flex-col h-full p-2 gap-2">
		<div
			class="flex items-center gap-2 p-3 rounded bg-neutral-900 border-l-4"
			:class="statusClass"
		>
		...
		>
			...
			>
				<div class="flex gap-1.5 mb-0.5">
					<span class="text-neutral-600 flex-shrink-0">{{
						entry.timestamp
					}}</span>
					<span class="text-neutral-500 flex-shrink-0 w-5 text-center">{{
						entry.direction
					}}</span>
					<span class="text-cyan-400 flex-shrink-0 font-medium">{{
						entry.event
					}}</span>
				</div>
