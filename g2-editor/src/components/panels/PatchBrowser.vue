<script setup lang="ts">
	...
	const emit = defineEmits<{
		select: [
			item:
				| { type: 'disk'; filepath: string }
				| { type: 'synth'; bank: number; location: number },
		];
	}>();
	...
	function groupByBank(
		list: SynthPatch[],
	): { bank: number; patches: SynthPatch[] }[] {
	...
	const filteredPatches = computed(() => {
		...
		return q
			? browser.synthPatches.filter((p) =>
					p.name.toLowerCase().includes(q),
				)
			: browser.synthPatches;
	});

	const filteredPerformances = computed(() => {
		const q = searchQuery.value.toLowerCase();
		return q
			? browser.synthPerformances.filter((p) =>
					p.name.toLowerCase().includes(q),
				)
			: browser.synthPerformances;
	});
	...
	const filteredDiskDirs = computed(() =>
		browser.diskEntries.filter((e) => e.isDir),
	);
	...
	const diskFolderName = computed(
		() => browser.diskFolder.split('/').pop() || browser.diskFolder,
	);
	...
	async function onViewChange(
		view: string | number | null | (string | number)[],
	) {
		const v = view as 'patches' | 'performances' | 'disk';
		browser.view = v;
		searchQuery.value = '';
		if (
			(v === 'patches' || v === 'performances') &&
			browser.synthPatches.length === 0 &&
			device.connected
		) {
	...
	function handleEnter() {
		if (browser.view === 'disk' && filteredDiskFiles.value.length > 0) {
			...
			browser.view === 'patches' &&
			filteredPatches.value.length > 0
		) {
			selectSynth(filteredPatches.value[0]);
		} else if (
			browser.view === 'performances' &&
	...
	function formatFileName(entry: DiskEntry): string {
		return entry.isDir
			? entry.name
			: entry.name.replace(/\.(pch2|prf2)$/i, '');
	}
</script>
...
<template>
	<div class="h-full flex flex-col overflow-hidden">
		...
		<template v-if="browser.view === 'disk'">
			<!-- Folder header -->
			<div
				class="flex items-center gap-1 px-2 py-1.5 border-b border-neutral-700 shrink-0 min-w-0"
			>
				<button
					v-if="diskParentName"
				...
				<span
					class="text-xs text-neutral-300 truncate flex-1 min-w-0 font-medium"
				>
					{{ diskFolderName || 'No folder selected' }}
				</span>
				<button
					class="text-sm text-neutral-400 hover:text-white px-1 shrink-0"
					title="Choose folder"
					@click="browser.chooseDiskFolder()"
				>
					…
				</button>
			</div>
			...
			<template v-if="browser.diskFolder">
				...
				<div
					v-if="browser.loading"
					class="p-4 text-center text-neutral-500 text-sm"
				>
					Loading...
				</div>
				<div
					v-else-if="browser.error"
					class="p-4 text-center text-red-500 text-xs"
				>
					{{ browser.error }}
				</div>
				<ul v-else class="flex-1 overflow-y-auto list-none m-0 p-0">
					...
					>
						<span
							class="text-neutral-600 text-xs shrink-0 w-3"
						></span>
						<span
							class="text-xs text-neutral-200 truncate flex-1"
							>{{ formatFileName(entry) }}</span
						>
					...
					<li
						v-if="
							filteredDiskDirs.length === 0 &&
							filteredDiskFiles.length === 0
						"
						class="p-4 text-center text-neutral-500 text-sm"
					>
		...
		<template v-else>
			<div
				...
				class="p-4 text-center text-neutral-500 text-sm"
			>
				Connect G2 to browse synth patches
			</div>
			<template v-else>
				<div class="px-2 pt-2 shrink-0">
					<SearchInput
				...
				<ul v-else class="flex-1 overflow-y-auto list-none m-0 p-0">
					...
					<template
						v-for="group in browser.view === 'patches'
							? patchGroups
							: perfGroups"
						:key="group.bank"
					>
						...
						<template v-if="!browser.isBankCollapsed(group.bank)">
							...
							>
								<span
									class="text-neutral-600 text-xs shrink-0 w-5"
									>{{ p.location }}</span
								>
								<span
									class="text-xs text-neutral-200 truncate flex-1"
