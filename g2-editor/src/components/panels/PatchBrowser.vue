...
const filteredPatches = computed(() => {
	...
	return q
		? browser.synthPatches.filter((p) => p.name.toLowerCase().includes(q))
...
const filteredPerformances = computed(() => {
	const q = searchQuery.value.toLowerCase();
	return q
		? browser.synthPerformances.filter((p) => p.name.toLowerCase().includes(q))
...
async function onViewChange(view: string | number | null | (string | number)[]) {
	...
	browser.view = v;
	searchQuery.value = "";
	if ((v === "patches" || v === "performances") &&
		browser.synthPatches.length === 0 && device.connected) {
...
function handleEnter() {
	if (browser.view === "disk" && filteredDiskFiles.value.length > 0) {
		emit("select", { type: "disk", filepath: filteredDiskFiles.value[0].path });
...
<template>
	<div class="h-full flex flex-col overflow-hidden">
		...
		<template v-if="browser.view === 'disk'">
			...
			<template v-if="browser.diskFolder">
				...
				<ul v-else class="flex-1 overflow-y-auto list-none m-0 p-0">
					...
					<li v-if="filteredDiskDirs.length === 0 && filteredDiskFiles.length === 0"
		...
		<template v-else>
			<div v-if="!device.connected" class="p-4 text-center text-neutral-500 text-sm">
			...
			<template v-else>
				...
				<div v-if="browser.loading" class="p-4 text-center text-neutral-500 text-sm">
					Loading from G2...
				</div>
				...
				<ul v-else class="flex-1 overflow-y-auto list-none m-0 p-0">
					...
					<template v-for="group in (browser.view === 'patches' ? patchGroups : perfGroups)"
						:key="group.bank">
					...
					<li v-if="(browser.view === 'patches' ? patchGroups : perfGroups).length === 0"
						class="p-4 text-center text-neutral-500 text-sm">
