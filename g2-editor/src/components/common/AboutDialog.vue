<template>
	<Teleport to="body">
		<div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="close">
			<div class="bg-neutral-700 border border-neutral-600 rounded shadow-xl w-88 mx-4">
				<div class="flex items-center justify-between px-4 py-3 border-b border-neutral-600">
					<span class="text-sm font-semibold text-neutral-200">About G2 Editor</span>
					<button class="text-neutral-400 hover:text-neutral-200 text-lg leading-none cursor-pointer" @click="close">×</button>
				</div>
				<div class="px-4 py-4 flex flex-col items-center gap-3 text-center">
					...
					<div>
						<div class="text-base font-bold text-neutral-100">G2 Editor</div>
						<div class="text-xs text-neutral-400 mt-0.5">v{{ version }}</div>
					</div>
					<div class="text-xs text-neutral-300">Modern Editor for Nord Modular G2</div>
					<div class="text-xs text-neutral-400">by Jan den Besten</div>
					...
					<div class="w-full border-t border-neutral-600 pt-3 flex flex-col gap-2 text-left">
				...
				<div class="flex justify-end px-4 py-3 border-t border-neutral-600">
