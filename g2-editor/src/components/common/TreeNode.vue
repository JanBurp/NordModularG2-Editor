...
<template>
  <div class="select-none" :style="{ paddingLeft: depth * 4 + 'px' }">
    <div class="flex items-center py-0.5 px-1 cursor-pointer rounded hover:bg-neutral-700" @click="handleToggle">
      ...
      <span v-if="!isExpandable(node)" class="text-green-500" :class="{ 'text-orange-400': getValueType(node.value) === 'number', 'text-cyan-400': getValueType(node.value) === 'boolean' }">{{ node.value }}</span>
