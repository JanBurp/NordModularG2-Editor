<template>
	<div class="relative">
		<div :style="currentColorStyle" class="border border-neutral-600 rounded min-w-10 h-8 cursor-pointer" @click="togglePicker()"></div>
		<div
			v-if="pickerOpen"
			class="z-50 absolute bg-neutral-200 grid grid-cols-4 grid-rows-4 border border-neutral-600 rounded w-40"
			@mouseleave="togglePicker()"
		>
			<template v-for="index in MODULE_COLORS_ORDER">
				<div v-if="index > 0" class="w-10 h-8 border border-neutral-600" :style="colorStyle(index)" @click="setColor(index)"></div>
			</template>
		</div>
	</div>
</template>
