<template>
	<div
		class="flex items-center h-6 gap-2 bg-neutral-500 border-b border-neutral-700 text-xs text-neutral-900"
	>
		<BtnGroup
		...
		/>
		<span
			>Voice: {{ areaCount('voice', 'modules') }} modules /
			{{ areaCount('voice', 'cables') }} cables</span
		>
		<StatusBarDivider />
		<span
			>FX: {{ areaCount('fx', 'modules') }} modules /
			{{ areaCount('fx', 'cables') }} cables</span
		>
		<div
