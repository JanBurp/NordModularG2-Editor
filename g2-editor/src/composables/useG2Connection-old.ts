...
export function useG2Connection() {
	...
	const g2Progress = ref<{
		state: string;
		percent: number;
		message: string;
	} | null>(null);
	const g2LoadedData = ref<{
		synthName: string;
		synthMode: string;
		slots: { name: string; version: number }[];
	} | null>(null);
	...
	function setupG2EventListeners() {
		if (typeof window !== "undefined" && window.electronAPI?.g2) {
			...
			unsubscribeConnectFailed = window.electronAPI.g2.onConnectFailed(
				(data) => {
					if (data.retries <= 1) {
						...
						log(
							"←",
							"Connect",
							`Auto-connect failed after ${data.attempt} attempts: ${data.error}`,
						);
					} else {
						log(
							"←",
							"Connect",
							`Auto-connect attempt ${data.attempt}/${data.retries} failed, retrying...`,
						);
					}
				},
			);
			...
			unsubscribeLoadedData = window.electronAPI.g2.onLoadedData((data) => {
				...
				for (let i = 0; i < data.slots.length; i++) {
					log(
						"←",
						"Data",
						`Slot ${String.fromCharCode(65 + i)}: ${data.slots[i].name || "(empty)"} (v${data.slots[i].version})`,
					);
				}
			});
		}
	...
	async function downloadFromG2(slot: number = 0): Promise<void> {
		...
		try {
			...
			log(
				"→",
				"Download",
				`Requesting patch from Slot ${String.fromCharCode(65 + slot)}...`,
			);
			...
			setTimeout(() => {
				...
				log(
					"←",
					"Download",
					`Patch request sent for Slot ${String.fromCharCode(65 + slot)}`,
				);
			}, 500);
		} catch (err) {
			const errorMessage = err instanceof Error ? err.message : "Unknown error";
