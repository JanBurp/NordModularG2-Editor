...
int g2_build_synth_set_msg(cJSON *params, uint8_t *out, size_t *out_len) {
    ...
    int sysexId        = (int)cJSON_GetNumberValue(cJSON_GetObjectItem(jMidi, "sysex"));
    int localOn        = cJSON_IsTrue(cJSON_GetObjectItem(jMidi, "local"));
    ...
    int clkSend        = cJSON_IsTrue(cJSON_GetObjectItem(jMidi, "clkse"));
    int clkRecv        = cJSON_IsTrue(cJSON_GetObjectItem(jMidi, "clkre"));
    ...
    out[base + 10] = (uint8_t)(sysexId - 1);
    out[base + 11] = localOn ? 0x80 : 0x00;
    out[base + 12] = (uint8_t)(prgch & 0x03);
    out[base + 13] = (uint8_t)(((ctrlsRecv & 1) << 1) | (ctrlsSend & 1));
    out[base + 14] = (uint8_t)((clkSend ? (1 << 6) : 0) | (!clkRecv ? (1 << 5) : 0));
